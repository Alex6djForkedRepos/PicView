...
/// </summary>
public class FileHistoryMenuController : IDisposable
{
    ...
    private void UpdateSortButtonIcon()
    {
        if (FileHistoryManager.IsSortingDescending)
        {
            if (Application.Current?.TryGetResource("SortDescImage",
                    Application.Current.RequestedThemeVariant, out var sortDescImage) == true)
            {
        ...
        else
        {
            if (Application.Current?.TryGetResource("SortAscImage",
                    Application.Current.RequestedThemeVariant, out var sortAscImage) == true)
            {
